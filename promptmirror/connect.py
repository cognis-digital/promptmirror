"""Native cognis-connect emit for promptmirror — forward findings to any platform.

Maps promptmirror's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    promptmirror ... --format json | promptmirror-emit --to stix
    promptmirror-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop("layered", None)
        out.pop("raw", None)
        out.pop("score", None)
        out.pop("risk", None)
        out.pop("verdict", None)
        out.pop("text_len", None)
        out.pop("max_severity", None)
        out.pop("counts", None)
        out.pop("owasp_breakdown", None)
        out.pop("decoded_layers", None)
        return out
    except Exception:
        return rec
