...
```mermaid
flowchart LR
  IN[target / manifest] --> P[promptmirror<br/>checks + rules]
...
`promptmirror` is interoperable with every popular way of using AI:

- **MCP server** — `promptmirror mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))
- **OpenAI-compatible / JSON** — pipe `promptmirror scan . --format json` into any agent or LLM
...
```bash
pip install "git+https://github.com/cognis-digital/promptmirror.git"    # pip (works today)
pipx install "git+https://github.com/cognis-digital/promptmirror.git"   # isolated CLI
uv tool install "git+https://github.com/cognis-digital/promptmirror.git" # uv
pip install cognis-promptmirror                                          # PyPI (when published)
docker run --rm ghcr.io/cognis-digital/promptmirror:latest --help        # Docker
brew install cognis-digital/tap/promptmirror                             # Homebrew tap
curl -fsSL https://raw.githubusercontent.com/cognis-digital/promptmirror/main/install.sh | sh
```
